...
We can now call `joinPlaylist` once the Playlist component has mounted. We maintain a list of items using `useState` and set the items once the playlist has been opened. Additionally we should listen for the `replicated` event which means a peer has added an item to the list, and reset the items.
...
Finally we can call the `sendFiles` method in `onDrop` and reset the items to reflect the new additions.
